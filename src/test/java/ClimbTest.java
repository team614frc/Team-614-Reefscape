...
class CanalTest {
  ...
  void climbTest() {
    ...
    assertEquals(Constants.ClimberConstants.CLIMB_SPEED, climber.getMotor().get());
  }

  @Test
  void reverseClimbTest() {
    climber.reverseClimb().execute();
    assertEquals(Constants.ClimberConstants.REVERSE_CLIMB_SPEED, climber.getMotor().get());
  }
}
