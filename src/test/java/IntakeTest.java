...
class IntakeTest {
  ...
  void intakeGamepieceTest() {
    ...
    assertEquals(Constants.IntakeConstants.INTAKE_SPEED, intake.getMotor().get());
  }

  @Test
  void pukeGamepieceTest() {
    intake.pukeGamepiece().execute();
    assertEquals(Constants.IntakeConstants.OUTTAKE_SPEED, intake.getMotor().get());
  }

  @Test
  void stopIntakeTest() {
    intake.stopIntake().execute();
    assertEquals(0.0, intake.getMotor().get());
  }
}
